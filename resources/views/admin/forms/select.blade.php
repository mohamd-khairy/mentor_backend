@if(isset($relations) && $relations)
    @if(isset($multiple) && $multiple)
        ...
        label-class="text-lightblue" igroup-size="sm" data-placeholder="{{__('cruds.'.$name)}}" multiple>
    ...
    @else
        <x-adminlte-select2  class="select2" name="{{$name}}" id="{{$id}}" label="{{__('cruds.'.$name)}}"
        label-class="text-lightblue" igroup-size="sm" data-placeholder="{{__('cruds.'.$name)}}">
            <x-slot name="prependSlot">
                <div class="input-group-text bg-lightblue">
                    <i class="fas fa-{{$icon ?? 'car-side'}}"></i>
                </div>
